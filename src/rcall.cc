...
std::string RCoreRuntime::getRsourceCode(const userFunction &userCode)
{
	...
	this->rLog.log(RServerLogLevel::LOGS, "the number of argment is %d", userCode.getFunctionArgsName().size());
...
ReturnStatus RCoreRuntime::setArgumentValues(const CallRequest *callRequest)
{
	...
	try
	{
		for (count = 0; count < callRequest->args_size(); count++)
		{
			...
			case PlcDataType::LOGICAL:
			{
				rArg = convter.boolToSEXP(callRequest->args()[count].scalarvalue().logicalvalue());
				break;
			}
			case PlcDataType::INT:
			{
			...
			case PlcDataType::TEXT:
			{
				rArg = convter.textToSEXP(callRequest->args()[count].scalarvalue().stringvalue());
				break;
			}
			case PlcDataType::BYTEA:
			{
				rArg = convter.byteaToSEXP(callRequest->args()[count].scalarvalue().byteavalue());
				break;
			}
			default:
				throw rArgumentErrorException("Unsupport type in args " + callRequest->args()[count].name());
				break;
			}
